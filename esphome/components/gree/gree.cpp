...
namespace esphome {
	namespace gree {
		...
		void GreeClimate::transmit_state_() {
			...
			// A known good state Power Off, Fan Auto, Mode Auto, 25C.
			remote_state[0] = 0x0;
			remote_state[1] = 0x09;
			remote_state[2] = 0x20;
			remote_state[3] = 0x50;
			...
			for (uint16_t r = 0; r <= repeat; r++) {
				// Here we need to send the first 4 bytes
				// The first 4 bytes have a header and no footer
				// then we send 0b010 with no header and a footer
				// then we send the last 4 bytes with no header and a footer
